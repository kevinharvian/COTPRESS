...
st.caption("Konversi gambar (termasuk JFIF/HEIC) & PDF ke JPG. Target ukuran otomatis: min 100 KB, max 170 KB. File video akan diabaikan (tidak dikompres) tanpa menghentikan proses.")
...
with st.sidebar:
    ...
    st.markdown("**Target otomatis:** 100–170 KB (tidak bisa diubah)")
...
PDF_EXT = {".pdf"}
# ✅ Video extensions — will be silently skipped (not compressed)
...
def process_one_file_entry(relpath: Path, raw_bytes: bytes, input_root_label: str):
    ...
    try:
        # ✅ Silently skip videos (do nothing, but track as skipped for summary)
        if ext in VIDEO_EXT:
            skipped.append((str(relpath), "Video — diabaikan"))
        ...
        # else: other unknown types silently ignored
    except Exception as e:
...
uploaded_files = st.file_uploader(
    "Upload beberapa ZIP (berisi folder/gambar/PDF) dan/atau file lepas (gambar/PDF). Video akan diabaikan otomatis.",
...
if run:
    ...
    for zname, zbytes in zip_inputs:
        try:
            ...
            # ✅ Sertakan juga video dalam items agar tercatat sebagai skipped, tapi tidak diproses
            allowed = IMG_EXT.union(PDF_EXT).union(VIDEO_EXT)
    ...
    if not jobs:
        st.error("Tidak ada berkas valid (butuh gambar/PDF/ZIP berisi file-file tersebut). Video saja tidak diproses."); st.stop()

    st.write(f"🔧 Ditemukan **{sum(len(j['items']) for j in jobs)}** berkas dari **{len(jobs)}** input (video akan diabaikan).")
    ...
    for job in jobs:
        ...
        with st.expander(f"📦 {base} — {len(items)} file diproses, {len(skipped)} dilewati/errored (termasuk video)"):
            ok = 0
            shown = 0
    ...
    st.success("Selesai! Master ZIP siap diunduh. Video diabaikan.")
